...
Additionally, due to imaginary budget restrictions I will not be evaluating the audience's perception of bubble plots.
...
The y-axis of the deceptive plot not only starts at \$11,00, but also stops at \$14,000.
...
**VERIFY THIS.** Note, the data points will be plotted correctly for all line plots, unlike in @fig-fox.
...
Questions about each plot will follow the format from @lauer where each participant will be asked to compare the difference or change from one state to another.  For the pie charts shown in @fig-pie this would be asking how many more romance movies were made than westerns.  For bar charts and line plots questions would follow the format in @fig-line.  This figure shows how a control line plot from the noncontroversial set would be presented.  The particpant is then asked how much change occured from one time point to another.
...
After completing the perception question for each plot, participants will be asked to rate the three types of plots in order of preference.  Examples of noncontroversial control plots will be shown in case they are still unfamiliar with the names of the plot types after completing the majority of the survey.


### Demographic Questions

Before completing the survey participants will be asked to fill out a demographic questionnaire.
The demographic variables will include gender, race, age group, region, income, education level, and political leaning.  
The question on their political leaning will try to capture a participant's feeling on where their opinions fall on the political spectrum.
...
The reason for including demographic information at the end of the survey instead of the beginning is related to the political leaning question.  If participants were asked this question prior to evaluating any plots, they may become aware of the survey's intent and answer questions differently than they otherwise would have.  By asking  demographic questions at the end of the survey the participants are more likely to provide genuine answers.

Note, all demographic questions and answer banks will be written with accessibility in mind.  Many questions will provide options for unlisted answers and a box for those who prefer not to answer.  This includes the political question.  While this question specifically could hinder the analysis in some ways, it way also allow us to evaluate apolitical participants along with those how fall on the political spectrum.

### START HERE - Set Order

**MUST WRITE THIS**

### Attention Checks

This survey will be conducted online.  To ensure participants are actively taking the survey, attention check questions will be used.  An example from @check is shown in @fig-att.  The question tells the participant which answer to select and hopefully filters out any participants that are simply selecting answers without reading the questions or evaluating the plots.  


![Example of an attention check. (@check).](att-check.png){#fig-att width="4.5in"}

Note, the format and timing of the attention checks are still non finalized.  Including unused plots with the checks may increase the odds of catching those who are not reading the questions.  Similarly, answers may be formatted to follow the number scale shown in @fig-line.

...
date-format: long
abstract: |
  This paper outlines an experiment designed to measure how people perceive information provided in plots and whether political bias affects their performance. This experiment shows participants a mix of plots with some being political or controversial topics, while others are more general.  Each plot has a control version and a deceptive version.  This allows us to evaluate how an audience percieves poor or misleading graphics compared to better ones.
execute: 
  echo: false
  warning: false
columns: 2
...
While this question specifically could hinder the analysis in some ways, it way also allow us to evaluate apolitical participants along with those how fall on the political spectrum.
...
**Updated Answer:** replace ```geom_point(color = "red", size = 2)``` with ```geom_text(aes(label = Dollars), vjust = -0.5, size = 3, color = "black")``` in previous code.
